module BranchPlanManager
  class BranchPlan3 < Base
    ...
    def promotions_or_manteinances_by_vehicle(promotion_type_class)
      result_relation = promotion_type_class.availables.actives.with_stock.not_deleted
        .joins(branches_promotions: :branch)
        .joins(promotions_vmes: {vme: {model: :brand}})
        .where("branches.id = ?", branch.id)

      if user_input.brand_id.present?
        result_relation = result_relation.where("marca.id_marca = ?", user_input.brand_id)
      end

      if user_input.model_id.present?
        result_relation = result_relation.where("modelo.id_modelo = ?", user_input.model_id)
      end
      ...
      if user_input.patent.present?
        ...
        if vehicle.vme.present?
          result_relation = result_relation.where("vehiculo_modelo_especifico.vme_id = ?", vehicle.vme.vme_id)
        end

        if user_input.kms.present?
          result_relation = result_relation.where("promotions.kms IS NOT NULL")
          result_relation = result_relation.order("abs(promotions.kms - #{user_input.kms})")
        end
      end
