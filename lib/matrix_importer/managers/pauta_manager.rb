  ...
  module Managers
    class PautaManager
      ...
      def create_or_update_generic_pauta
        branch_id = branch_sheet_manager.branches_ids.first
        xls_kms = kms_by_branch_id(branch_id)
        Vme.includes(:model).where(vme_id: vme_sheet_manager.vmes_ids).each do |vme|
          db_pautas_kms = vme.pautas.map{|pauta|pauta.kilometraje}.sort
          ...
          self.pautas += create_new_vme_pautas(vme, xls_kms, db_pautas_kms, branch_id)
          self.pautas += update_existing_pautas(vme, xls_kms, db_pautas_kms, branch_id)
          destroy_old_vme_pautas(vme, xls_kms, db_pautas_kms)
        end
      end

      def create_or_update_each_branch_pauta
        branch_sheet_manager.branches do |name, branch_id, sheet|
          self.pautas.each do |pauta|
            sheet = branch_sheet_manager.sheet(branch_id)
      ...
      private
        ...
        def create_new_vme_pautas(vme, xls_kms, db_pautas_kms, branch_id)
          pautas_to_create  = []
          ...
          columns = [:pauta_descripcion, :kilometraje, :id_marca, :id_modelo, :vme_id]
        ...
        def update_existing_pautas(vme, xls_kms, db_pautas_kms, branch_id)
          (pautas_to_update = Pauta.where(vme_id: vme.id, kilometraje: (xls_kms & db_pautas_kms))).each do |pauta|
            sheet = branch_sheet_manager.sheet(branch_id)
            new_manteinance_items = manteinance_item_manager.manteinance_items_and_prices_by_km_by_sheet(pauta.kilometraje, sheet).map{|h| h[:obj]}
        ...
        def destroy_old_vme_pautas(vme, xls_kms, db_pautas_kms)
        ...
        def kms_by_branch_id(branch_id)
          branch_sheet_manager.branches do |name, _branch_id, sheet|
