class CouponCreatedMailer < ApplicationMailer
  ...
  def notify_client(coupon, promotions_ids, patent, kms)
    template_name = (coupon.promotion.info_modal? or coupon.promotion.is_a?(BranchInformation)) ? 'mismantenciones-com-cupon-information' : 'mismantenciones-com-cupon'
  ...
  def notify_shop(coupon, patent, kms)
    template_name = (coupon.promotion.info_modal? or coupon.promotion.is_a?(BranchInformation)) ? 'mismantenciones-com-cupon-branch-information' : 'mismantenciones-com-cupon-branch'
    subject = (coupon.promotion.info_modal? or coupon.promotion.is_a?(BranchInformation)) ? "Cliente: #{coupon.client.full_name} ha solicitado más información en #{coupon.promotion.branches.first.try(:shop).try(:name)}." : "#{coupon.client.full_name} ha adquirido un cupón de servicio en #{coupon.promotion.branches.first.try(:shop).try(:name)}."
    template_content = []
    message = {
      from_name: default_from_name,
      from_email: default_from_email,
